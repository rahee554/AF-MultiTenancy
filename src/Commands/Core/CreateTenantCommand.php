...
BLADE;
    ...
    private function createWithFastPanel(array $tenantData): int
    {
        try {
            ...
            // Step 5: Ask user about database - select existing or create new
            $this->newLine();
            ...
            if ($dbChoice === 'existing') {
                ...
                $selectedDb = $this->selectExistingDatabase($dbUser);
                if (!$selectedDb) {
                    $this->error('❌ No database selected');
                    return 1;
                }
                $dbDetails['name'] = $selectedDb;
                $skipDatabaseCreation = true; // Database already exists
                $this->info("📂 Using existing database: {$dbDetails['name']}");
            } else {
                // Generate database details for new database
                $dbDetails = $this->generateDatabaseDetails($tenantData['name'], $tenantData['database']);

                if (!$dbDetails['name']) {
                    $this->info("📊 Database name will be auto-generated by TenantService");
                    ...
                    $exists = $this->databaseExists($dbDetails['name'], $dbUser);
                    if ($exists) {
                        $this->warn("⚠️  Database '{$dbDetails['name']}' already exists.");
                        if (!$this->confirm('Use this existing database?', true)) {
                            $this->error('❌ Tenant creation cancelled');
                            return 1;
                        }
                ...
                // Step 8: Handle FastPanel database user assignment (always do this for FastPanel databases)
                if ($dbDetails['name']) {
            ...
            if (!$dbDetails['name']) {
                ...
                $this->info("✅ Database auto-generated and created by TenantService: {$dbDetails['name']}");
    ...
    private function createWithLocalhost(array $tenantData): int
    {
        try {
            ...
            if ($dbDetails['name']) {
                ...
                $this->info("🔧 Database will be auto-generated and created...");
    ...
    private function databaseExists(string $dbName, array $dbUser): bool
    {
        try {
            ...
            // Use INFORMATION_SCHEMA instead of SHOW DATABASES for proper parameter binding
            $databases = DB::connection($connection)->select('SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = ?', [$dbName]);
            return !empty($databases);
        } catch (Exception $e) {
            $this->warn("Could not check if database exists: {$e->getMessage()}");
            return false;
        }
    }

    private function selectExistingDatabase(array $dbUser): ?string
    {
        $this->info('📂 Fetching existing databases...');
        ...
        try {
            ...
            $allDatabases = DB::connection($connection)->select('SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA ORDER BY SCHEMA_NAME');
            ...
            if (empty($databases)) {
                $this->warn('⚠️  No user databases found');
                return null;
            }
            ...
            $choices = ['cancel' => '❌ Cancel - Create new database instead'];
            foreach ($databases as $idx => $db) {
                $choices[$idx] = $db;
                $this->line("  <info>{$idx}</info>: {$db}");
            }

            $this->newLine();
            $selected = $this->choice('Select existing database to use', $choices, 'cancel');
            ...
            $this->error("Error fetching databases: {$e->getMessage()}");
            return null;
        }
    }
